...
The loop is unraveled to allow fast indexing;
*/
...
void initializeCustomQueues(){
    for (int i = 0; i < MAX_NUM_INPUT_THREADS; i++){
        queues[i].segment[0].ptr = queues[i].segment[0].buffer;
        queues[i].segment[1].ptr = queues[i].segment[1].buffer;
    }
}
...
void * input_thread(void * args){
    ...
    vbseg_t* shared1 = &queues[threadIndex].segment[0];
    vbseg_t* shared2 = &queues[threadIndex].segment[1];
    ...
    while(1){
        ...
        while(1){
            ...
            if ((local.ptr - local.buffer + MAX_PACKET_SIZE) >= BUFFSIZEBYTES) {
                //If there's still data in the shared buffer, wait
                while (shared2->ptr > shared2->buffer) {
                    ;
                }
                //Copy the entire vector to shared memory
                memcpy(shared2->buffer, local.buffer, (local.ptr - local.buffer));
                //Signal to output_thread there's more data in shared memory and how much
                shared2->ptr = shared2->buffer + (local.ptr - local.buffer);
                //Reset the local queue
                local.ptr = local.buffer;
                break;
            }
...
void * output_thread(void * args){
    ...
    vbseg_t* shared1 = &queues[qIndex].segment[0];
    vbseg_t* shared2 = &queues[qIndex].segment[1];
    ...
    while(1){
        ...
        shared1 = &queues[qIndex].segment[0];
        ...
        //Copy the entire vector from shared to local memory
        memcpy(local.buffer, shared2->buffer, (shared2->ptr - shared2->buffer));

        //local.ptr marks where data in the local buffer ends
        local.ptr = local.buffer + (shared2->ptr - shared2->buffer);

        //Signal to input_thread that shared memory can be written to again
        shared2->ptr = shared2->buffer;
        ...
        //Assign the second segment to the new queue's second segment
        shared2 = &queues[qIndex].segment[1];
    }
