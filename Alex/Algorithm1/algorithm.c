...
void grabPackets(int toGrabCount, queue_t* mainQueue){
    ...
    for(int qIndex = 0; qIndex < input.queueCount; qIndex++){
        for(int packetCount = 0; packetCount < toGrabCount; packetCount++){
            ...
            if(input.queues[qIndex].data[inputReadIndex].isOccupied == NOT_OCCUPIED){
                break;
            }

            //Grab the packets data and move it into the mainQueue
            (*mainQueue).data[mainWriteIndex].packet.flow = input.queues[qIndex].data[inputReadIndex].packet.flow;
            (*mainQueue).data[mainWriteIndex].packet.order = input.queues[qIndex].data[inputReadIndex].packet.order;
...
void passPackets(queue_t* mainQueue){
    ...
    while(1){
        ...
        int qIndex = (*mainQueue).data[mainReadIndex].packet.flow % output.queueCount;
        ...
        output.queues[qIndex].data[outputWriteIndex].packet.order = (*mainQueue).data[mainReadIndex].packet.order;
        output.queues[qIndex].data[outputWriteIndex].packet.flow = (*mainQueue).data[mainReadIndex].packet.flow;
